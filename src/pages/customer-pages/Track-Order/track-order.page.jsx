    ...
    className="max-w-[1000px] mx-auto mt-[40px] p-4 bg-[#141414] rounded-xl px-[30px] py-[20px]">
      ...
      <div className="relative flex items-center justify-between mb-4 mt-[20px]">
        <div className="absolute top-1/2 transform -translate-y-5 w-full h-2 bg-gray-300 rounded-lg">
          <div className={`h-2 ${getProgressClass(1)} rounded-tl-xl rounded-bl-xl rounded-tr-xl rounded-br-xl  `} style={{ width: `${calculateProgress(selectedOrder.order_status)}%` }}></div>
        </div>
      ...
      <div className="mt-2">
        ...
        <p className="text-white font-inter mb-2">Order ID:{selectedOrder.order_id}</p>
        <p className="text-white font-inter mb-2">Delivery Address: {selectedOrder.delivery_address}</p>
        <p className='text-white font-inter mb-2'>Total Price: {selectedOrder.total_price}$</p>
        ...
        <ul className="text-white font-inter mb-4 max-h-[200px] overflow-y-auto">
          {selectedOrder.order_items.map((item, index) => (
            <li key={index} className="flex items-center">
              <img src={item.imageURL} alt={item.productName} className="w-16 h-16 object-cover rounded-lg mr-4" />
